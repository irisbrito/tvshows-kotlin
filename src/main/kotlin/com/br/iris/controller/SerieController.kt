...
class SerieController(private val serieService: SerieService) {
    ...
    fun addSerie(@Body series: Serie): HttpResponse<Serie?> =
        HttpResponse.created(HttpStatus.CREATED).body(this.serieService.create(series))
    ...
    fun updateSerie(@PathVariable id:UUID, @Body serie: Serie): HttpResponse<Serie> =
        HttpResponse.ok(HttpStatus.OK).body(serieService.update(id, serie))

    @Delete("/{id}")
    fun deleteSerie(@PathVariable id:UUID) = serieService.delete(id)
